...
void callback_camera(const sensor_msgs::ImageConstPtr &msg);
...
int main(int argc, char **argv) {
  ...
  // Launch our ros node
  ros::init(argc, argv, "ros1_bee_msckf");
  ...
  if (!parser->successful()) {
    PRINT_ERROR(RED "[BEE]: unable to parse all parameters, please fix\n" RESET);
  ...
  // nh->param<std::string>("topic_imu", topic_imu, "/mavros/imu/data_raw");
  // parser->parse_external("relative_config_imu", "imu0", "rostopic", topic_imu);
  PRINT_DEBUG("[BEE]: imu: %s\n", topic_imu.c_str());
  // ros::Subscriber sub_imu = nh->subscribe(topic_imu, 1, callback_imu);
  ros::Subscriber sub_imu = nh->subscribe(topic_imu, 0, callback_imu);
  ...
  ros::Subscriber sub_camera = nh->subscribe(cam_topic, 0, callback_camera);
  // ros::Subscriber sub_camera = nh->subscribe(cam_topic, 1, callback_camera);
  /*
  // Our camera topics
  ...
  //   topic_cameras.emplace_back(cam_topic);
  //   PRINT_DEBUG("[BEE]: cam: %s\n", cam_topic.c_str());
  //   ros::Subscriber sub_camera = nh->subscribe(cam_topic, 1, callback_camera);
  //   // ros::Subscriber sub_imu = nh->subscribe(cam_topic, 1, viz->callback_inertial);
  // }
...
void callback_camera(const sensor_msgs::ImageConstPtr &msg) {
  ROS_INFO( "%dx%d", msg->width, msg->height);
